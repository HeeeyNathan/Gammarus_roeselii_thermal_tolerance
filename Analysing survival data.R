...
survdat <- read.csv("gammroes_temp_survival_LF_HD.csv")

# create sensoring variable (right censoring)
survdat$censored[survdat$surv == 0] <- 0
survdat$censored[survdat$surv == 1 | survdat$surv == 2 | survdat$surv == 3 | survdat$surv == 4 | survdat$surv == 5] <- 1

# Inspect distribution of LOS variable
hist(survdat$temp)
hist(subset(survdat, surv == 0)$temp)
hist(subset(survdat, surv == 1)$temp)
hist(subset(survdat, surv == 2)$temp)
hist(subset(survdat, surv == 3)$temp)
hist(subset(survdat, surv == 4)$temp)
hist(subset(survdat, surv == 5)$temp)

# Kaplan-Meier Analysis - life table
library(survival)

# specify KM analysis model
km_fit1 <- survfit(Surv(temp, censored) ~ 1, # first argument = time to event variable, second 
...
summary(km_fit1, times = c(22, 25, 28, 31, 34)) # specifies time intervals of interest (90 day increments)
...
km_fit2 <- survfit(Surv(temp, censored) ~ motu, # first argument = time to event variable, second 
                   data = survdat,          # argument = survival function. Adding 1 after tilda 
                   type = "kaplan-meier")   # makes null model, if you want a categorical variable, add it there.
print(km_fit2)
...
summary(km_fit2, times = c(22, 25, 28, 31, 34)) # specifies time intervals of interest (90 day increments)
...
survfit2(Surv(temp, censored) ~ motu, data = survdat) %>% 
  ggsurvfit() +
  labs(
    x = "Temperature",
...
cox_reg1 <- coxph(Surv(temp, censored) ~ motu,
...
# estimate cox proportional hazards (ph) model (cox regression model)
cox_reg2 <- coxph(Surv(temp, censored) ~ motu + pop + rep + run,
